...
「ページ → アニメーション（タイムライン）」で、ページを短いアニメーションにできます（セル・タイムライン・前後のセルを透かす・カメラワーク・GIF や MP4 に書き出し）。
「ファイル → 開く…」で開いた原稿はタブに並び、Ctrl+Tab で切り替えます。「ウィンドウ → 新しいウィンドウ（同じ原稿）」で、拡大して描く窓と全体を見る窓を並べられます。
