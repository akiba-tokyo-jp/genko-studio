...
_ERRORS: list[tuple[str, object]] = [
    ...
    (r"revoke needs a person", "承認の取り消しは人だけが行えます"),
    (r"reject_sheet needs a person", "設定画を却下できるのは人だけです"),
    (r"only a person can change the policy", "進め方の設定（policy）を変えられるのは人だけです"),
    (r"only a person can set prompt/size overrides", "プロンプトや寸法の上書きは人だけが指定できます"),
    (r"only a person can mark a panel skip", "コマを「絵なし」にできるのは人だけです"),
    (r"only a person can unpin", "固定を外せるのは人だけです"),
    (r"(\w+) is pinned by a person", r"\1 は人が固定しています"),
    (r"a person drew this region; only a person can change it", "人が描いた範囲なので、変えられるのは人だけです"),
    (r"the character sheet is approved; only a person can change it", "設定画は承認済みなので、変えられるのは人だけです"),
