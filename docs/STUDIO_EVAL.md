...
目的: 人間が描いたアタリから、コマ割りがどれだけ正しく復元されるか。スキャンしたアタリ 20 ページ以上と、その正解（コマの矩形 mm）を用意する。目標は、コマの 90% 以上が 5 mm 以内。

1. アタリをスキャンする（150〜300 dpi、グレースケールでよい）。紙全体を写すなら `align: "page"`、描いた範囲だけなら `"live"`、分からなければ `"auto"`。
2. 正解を書く（B4 商業原稿以外は `page_mm` を足す）:

   ```json
   {"align": "page", "page_mm": [257, 364],
    "pages": [{"scan": "scans/p01.png", "panels": [[13, 13, 231, 100], [13, 119, 110, 110]]}]}
   ```

   矩形は `[x, y, 幅, 高さ]`（mm、紙の左上から、枠線の中心）。
3. 測る: `genko studio eval-atari truth.json`。`rate` が 0.9 以上なら合格（`passed: true`）。`pages[]` の `detected` と `matched` で、外れたページを確かめ、`genko studio analyze PROJ --page N --params '{"min_gutter_mm": 1.0}'` などで調整する。
